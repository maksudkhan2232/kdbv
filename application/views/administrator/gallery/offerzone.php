                      ...
                      ?>
                        <tr>
                          ...
                          <td><?php if($val['status'] == 1){ ?>
                            ...
                            <?php } else { ?>
                            <button type="button" class="btn btn-sm btn-toggle changestatus" data-table="offerzone" data-field="status" data-id-name="id" data-id="<?php echo $val['id'];?>" data-toggle="button" aria-pressed="0" autocomplete="off">
                            <div class="handle"></div>
                            </button>
                            <?php } ?>
                          </td>
                          <td><a href="<?php echo base_url(); ?>administrator/offerzone/edit_offerzone/<?php echo $val['id']; ?>" class="btn btn-outline-primary mb-2">Edit</a>&nbsp;&nbsp;<a href="javascript:void(0);" onClick="check_confirm_delete('<?php echo $val['id']; ?>');" class="btn btn-outline-danger">Delete</a></td>
                        </tr>
                      <?php
                      $i++;
                      } } ?>
...
<script type="text/javascript">
  function check_confirm_delete(row_id)
  {
    swal({
        ...
        .then((willDelete) => {
          if (willDelete) {
              window.location.href = "<?php echo base_url() ?>"+"administrator/offerzone/delete_offerzone/"+row_id;
          } else {
